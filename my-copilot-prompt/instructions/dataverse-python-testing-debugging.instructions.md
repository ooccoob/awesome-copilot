...
# 适用于 Python 的 Dataverse SDK — 测试和调试策略

基于官方 Azure Functions 和 pytest 测试模式。

## 1. 测试概述

### 测试 Dataverse SDK 的 Pyramid
...
|模拟未调用 |检查方法名称/参数是否完全匹配 |
|真实 API 失败 |检查凭据、URL、权限 |
|测试中的速率限制 |添加延迟或使用较小的批次 |
|未找到数据 |验证已创建且未清理的记录 |
|断言错误 |打印实际值与预期值 |
...
- [Azure 函数测试](https://learn.microsoft.com/en-us/azure/azure-functions/functions-reference-python#unit-testing)
- [Dataverse SDK示例](https://github.com/microsoft/PowerPlatform-DataverseClient-Python/tree/main/examples)
