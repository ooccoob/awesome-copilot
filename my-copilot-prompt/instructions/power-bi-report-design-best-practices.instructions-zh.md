---
描述：“基于 Microsoft 指南的全面 Power BI 报告设计和可视化最佳实践，用于创建有效、可访问且高性能的报告和仪表板。”
applyTo: '**/*.{pbix,md,json,txt}'
---

# Power BI 报表设计和可视化最佳实践

## 概述
本文档提供了有关按照 Microsoft 官方指南和用户体验最佳实践设计有效、可访问且高性能的 Power BI 报告和仪表板的全面说明。

## 基本设计原则

### 1.信息架构
**视觉层次结构** - 按重要性组织信息：
- **主要**：关键指标、KPI、最关键的见解（左上角、标题区域）
- **次要**：支持细节、趋势、比较（主体）
- **第三级**：过滤器、控件、导航元素（侧边栏、页脚）
...
请记住：在设计时始终牢记您的特定用户和用例。尽早并经常使用真实用户和真实数据进行测试，以确保您的报告有效地传达见解并实现数据驱动的决策。