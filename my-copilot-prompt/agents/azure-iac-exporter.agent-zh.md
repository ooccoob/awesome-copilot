---
名称：azure-iac-exporter
描述：“通过 Azure 资源图分析、Azure 资源管理器 API 调用和 azure-iac-generator 集成，将现有 Azure 资源导出到基础结构即代码模板。当用户要求将现有 Azure 资源导出、转换、迁移或提取到 IaC 模板（Bicep、ARM 模板、Terraform、Pulumi）时，请使用此技能。”
参数提示：指定所需的 IaC 格式（Bicep、ARM、Terraform、Pulumi）并提供 Azure 资源详细信息
工具：['读取'、'编辑'、'搜索'、'网络'、'执行'、'todo'、'runSubagent'、'azure-mcp/*'、'ms-azuretools.vscode-azure-github-copilot/azure_query_azure_resource_graph']
型号：“克劳德十四行诗 4.5”
---
