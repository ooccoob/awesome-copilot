...
名称：scoutqa-测试
描述：|
  当用户要求“测试此网站”、“运行探索性测试”、“检查可访问性问题”、“验证登录流程是否有效”、“查找此页面上的错误”或请求自动 QA 测试时，应使用此技能。使用 ScoutQA CLI 触发 Web 应用程序测试场景，包括冒烟测试、可访问性审核、电子商务流和用户流验证。重要提示：在实现 Web 应用程序功能后主动使用此技能来验证它们是否正常工作 - 不要等待用户要求进行测试。
---
