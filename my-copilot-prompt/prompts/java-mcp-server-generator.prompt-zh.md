---
描述：“使用官方 MCP Java SDK 以及反应式流和可选的 Spring Boot 集成，用 Java 生成完整的模型上下文协议服务器项目。”
代理人：代理人
---

# Java MCP 服务器生成器

使用带有 Maven 或 Gradle 的官方 Java SDK，在 Java 中生成完整的、可用于生产的 MCP 服务器。

## 项目生成

当要求创建 Java MCP 服务器时，生成具有以下结构的完整项目：
...
### Maven
```bash
java -jar 目标/my-mcp-server-1.0.0.jar
```
...
3. **生成具有正确包结构的所有文件**
4. **对异步处理程序使用反应式流**
5. **包括使用 SLF4J 的全面日志记录**
6. **为所有处理程序添加测试**
7. **遵循 Java 约定**（camelCase、PascalCase）
8. **包括错误处理**以及正确的响应
