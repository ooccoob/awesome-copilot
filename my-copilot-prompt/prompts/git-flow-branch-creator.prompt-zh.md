---
描述：“智能 Git Flow 分支创建器，可分析 git status/diff 并按照 nvie Git Flow 分支模型创建适当的分支。”
...
1. 运行 `git status` 查看当前存储库状态和更改的文件。
2. 运行 `git diff` （对于未暂存的更改）或 `git diff --cached` （对于暂存的更改）来分析更改的性质。
3. 使用下面的 Git Flow Branch Analysis Framework 分析更改。
4. 根据分析确定适当的分支类型。
5. 遵循 Git Flow 约定生成语义分支名称。
6. 创建分支并自动切换到该分支。
7. 提供分析摘要和后续步骤。
