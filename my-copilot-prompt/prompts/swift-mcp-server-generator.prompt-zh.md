---
描述：“使用官方 MCP Swift SDK 包在 Swift 中生成完整的模型上下文协议服务器项目。”
代理人：代理人
---

# Swift MCP 服务器生成器

使用官方 Swift SDK 包在 Swift 中生成完整的、可用于生产的 MCP 服务器。

## 项目生成

当要求创建 Swift MCP 服务器时，生成具有以下结构的完整项目：
...
1. **询问项目名称和描述**
2. **生成所有文件**并具有正确的命名
3. **使用基于参与者的状态**来保证线程安全
4. **包括使用 swift-log 进行全面日志记录**
5. **使用 ServiceLifecycle 实现正常关闭**
6. **为所有处理程序添加测试**
7. **使用现代 Swift 并发**（异步/等待）
8. **遵循 Swift 命名约定**（camelCase、PascalCase）
9. **包括错误处理**以及正确的 MCPError 使用
10. **使用文档注释记录公共 API**
