---
description: '使用官方MCP Java SDK生成完整的Java MCP服务器项目，支持响应式流和可选的Spring Boot集成。'
mode: agent
---

# Java MCP服务器生成器

使用官方Java SDK和Maven或Gradle生成一个完整的、生产就绪的Java MCP服务器。

## 项目生成

当被要求创建Java MCP服务器时，生成具有以下结构的完整项目：
...
使用Java和官方MCP Java SDK构建的模型上下文协议服务器。
...
5. **包含全面的日志记录**，使用SLF4J
6. **添加测试**给所有处理器
7. **遵循Java约定**（camelCase、PascalCase）
8. **包含错误处理**，提供正确的响应
9. **记录公共API**，使用Javadoc
10. **提供同步和异步**示例